...
CUDA_VISIBLE_DEVICES=$cuda_idx torchrun --standalone --nproc_per_node $cuda_num torchrun_main_DDP.py \
    --model_name "fosl rank:$rank ngpu:$cuda_num lr:$lr folding_ratio:$folding_ratio lr_act:$lr_act sts:$sparse_trainable_scaling mix_trainable:$mix_trainable mix_per_channel:$mix_per_channel mix_init:$mix_init init_lost:$init_lost" \
    --wandb_project_name "Efficient_Pretraining" \
    --wandb_group_name "llama_130m" \
